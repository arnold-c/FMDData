...
@testset verbose = true "pre-post-checks.jl" begin
    ...
    # This test checks the full error message using a regex and occursin, which supports regex in Julia.
    @test occursin(
        r"All serotype results should have both 'Pre' and 'Post' results columns, only\. Instead, the following serotype results have the associated data columns:\s*(OrderedCollections\.)?OrderedDict\{AbstractString, Vector\{AbstractString\}\}\(\"serotype_a_pct\" => AbstractString\[\"pre\"\], \"serotype_a_count\" => AbstractString\[\"post\"\]\)",
        Try.unwrap_err(check_pre_post_exists(missing_pre_post_df))
    )
